...
function getCategoryLabel(category: TopicCategory, locale: string) {
...
export function ExpandedFihrist({ open, onClose, chapters }: ExpandedFihristProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return EXPANDED_TOPIC_INDEX.map((cat) => ({
      ...cat,
      topics: cat.topics.filter((entry) =>
        entry.topic.toLowerCase().includes(q) || entry.topicEn.toLowerCase().includes(q),
      ),
      ...
      >
        ...
        <div className="sticky top-0 z-10 flex-shrink-0 rounded-t-2xl bg-[var(--theme-bg-secondary)] px-5 pb-3 pt-5">
          ...
          <p className="mt-2 text-[12px] text-[var(--theme-text-tertiary)]">
            {search
              ? t.browse.resultCount.replace("{count}", String(totalTopics))
              : t.browse.categoryStats.replace("{categories}", String(filtered.length)).replace("{topics}", String(totalTopics))}
          </p>
        </div>
